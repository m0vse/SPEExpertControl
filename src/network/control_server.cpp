...
private:
    ...
    void sendIndex(WiFiClient &client)
    {
        ...
        client.println(F("function receive(a){setTop('',div('line sel',330,126,' Standby ')+'<img class=\"speLogo\" src=\"/spe-logo.svg\">'+div('line',470,45,'EXPERT 1K-FA<br>Solid State<br>Fully Automatic'));setAmp(a,true)}function tx(a){let op=a.screenName=='operate_rx'||a.screenName=='operate_tx';setTop('',div('line sel',323,126,op?' Operate ':' TX Exciter ')+ticks(136,2,420,a.meterPowerScale||[])+div('line',7,20,e(a.meterPowerLabel))+bar(136,22,420,a.meterPowerMax,a.meterPower)+rdiv('value',568,20,185,e(a.meterPower)+e(a.meterPowerSuffix))+ticks(136,64,420,a.meterPaScale||[])+div('line',7,84,e(a.meterPaLabel))+bar(136,86,420,a.meterPaMax,a.meterPa)+rdiv('value',568,84,185,e(a.meterPa)+e(a.meterPaSuffix)));setAmp(a,true)}"));
