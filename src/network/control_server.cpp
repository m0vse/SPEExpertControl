...
private:
    ...
    void sendIndex(WiFiClient &client)
    {
        ...
        client.println(F("function s(id,v){document.getElementById(id).textContent=v}function e(v){return String(v==null?'':v).replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}function div(c,x,y,t){return '<div class=\"'+c+'\" style=\"left:'+x+'px;top:'+y+'px\">'+t+'</div>'}function rdiv(c,x,y,w,t){return '<div class=\"'+c+'\" style=\"left:'+x+'px;top:'+y+'px;width:'+w+'px\">'+t+'</div>'}function ticks(x,y,w,a){let h='';for(let i=0;i<a.length;i++)h+=rdiv('scale',Math.round(x+i*w/(a.length-1)-35),y,70,e(a[i]));for(let i=0;i<21;i++){let maj=i%5==0;h+='<div class=\"tick\" style=\"left:'+Math.round(x+i*w/20)+'px;top:'+(y+14+(maj?0:4))+'px;height:'+(maj?8:4)+'px\"></div>'}return h}function bar(x,y,w,max,val){let p=Math.max(0,Math.min(100,Math.round((Number(val)||0)*100/max)));return '<div class=\"bar\" style=\"left:'+x+'px;top:'+y+'px;width:'+w+'px\"><div class=\"fill\" style=\"width:'+p+'%\"></div></div>'}function pF(st){let w=[5.2,6.5,12.5,21,40.9,86.5,168,342,693.5,1384.5],r=((st[3]&3)<<8)|st[2],v=0;for(let i=0;i<10;i++)if(r&(1<<i))v+=w[i];return v.toFixed(1)}"));
        ...
        client.println(F("function receive(a){setTop('',div('line sel',330,126,' Standby ')+'<img class=\"speLogo\" src=\"/spe-logo.svg\">'+div('line',470,45,'EXPERT 1K-FA<br>Solid State<br>Fully Automatic'));setAmp(a,true)}function tx(a){let op=a.screenName=='operate_rx'||a.screenName=='operate_tx',os=a.opStatus||0,out=os==1,full=a.out=='FULL',pmax=full?1200:600,ps=full?['0','300','600','900','1200']:['0','150','300','450','600'],vmax=(!out&&full)?60:50,vs=(!out&&full)?['0','15','30','45','60']:['0','12.5','25','37.5','50'],pv=out?a.power:a.reverse,sv=out?a.current:a.voltage;setTop('',div('line sel',323,126,op?' Operate ':' TX Exciter ')+ticks(136,2,420,ps)+div('line',7,20,out?'PW OUT':'PW REV')+bar(136,22,420,pmax,pv)+rdiv('value',568,20,185,e(pv)+' W pep')+ticks(136,64,420,vs)+div('line',7,84,out?'  I PA':'V PA')+bar(136,86,420,vmax,sv)+rdiv('value',568,84,185,e(sv)+(out?' A':' V')));setAmp(a,true)}"));
