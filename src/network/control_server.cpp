...
private:
    ...
    void sendIndex(WiFiClient &client)
    {
        ...
        client.println(F("let busy=false,fail=0;async function u(){if(busy)return;busy=true;try{let r=await fetch('/api/status',{cache:'no-store'});if(!r.ok)throw Error(r.status);let j=await r.json(),a=j.amp,w=j.wifi;fail=0;s('screen',a.valid?a.screenName:'WAITING FOR AMP');render(a);s('wifi',w.ssid||w.status);s('ip',w.ip);s('rssi',w.rssi+' dBm')}catch(e){if(++fail>=4){s('screen','OFFLINE');setTop('',div('line sel',330,65,' OFFLINE '));setAmp({},false)}}finally{busy=false}}async function k(b){b.classList.add('hit');try{await fetch('/api/key?name='+b.dataset.k,{cache:'no-store'})}finally{setTimeout(()=>b.classList.remove('hit'),160);setTimeout(u,250)}}document.querySelectorAll('button[data-k]').forEach(b=>b.onclick=()=>k(b));u();setInterval(u,1500)</script></body></html>"));
    }
